...
test('historical branding and full international phone have one spoken source', () => {
  assert.equal(BRAND.phone,'+221 76 944 48 49');
  assert.equal(BRAND.slogan,'Kër gu baax, xel mu dal');
  assert.equal(BRAND.website,'jammimmo.com');
  assert.match(OUTRO[1].voiceoverFr,/plus deux cent vingt et un, soixante-seize, neuf cent quarante-quatre, quarante-huit, quarante-neuf/);
...
test('fresh composition has one natural-rate track, bounded drawing and end-only website',async()=>{
  ...
  assert.match(source,/height:580,overflow:'hidden'/);
  assert.equal((source.match(/jammimmo\.com/g)||[]).length,2); // end-card and equivalent numeric subtitle only
...
test('manual prototype is isolated from the all matrix and preserves exact narration',async()=>{
  ...
  assert.equal(spec.id,'fresh-compteurs-v2');
